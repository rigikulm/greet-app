...
export default async (event: any, context: any) => {
  ...
  if (event.queryStringParameters.hasOwnProperty('lang')) {
    lang = <string>event.queryStringParameters.lang;
  }
  ...
  try {
    ...
    if (results.Item) {
      ...
      log.warn(httpStatus(400), `Warning: lang: ${lang}, phrase: ${phraseId} not found`);
